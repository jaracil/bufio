...
void bufio_extend(bufio_t *p, size_t sz) {
	if (sz > bufio_maxblk(p)) sz = bufio_maxblk(p);
	p->wp += sz;
	p->buf[p->wp] = 0;
}
...
size_t bufio_push_bytes(bufio_t *p, const void *s, size_t sz){
	if (bufio_avail(p) < sz) sz = bufio_avail(p);
	if(bufio_maxblk(p) < sz) bufio_shift(p);
...
size_t bufio_push_byte(bufio_t *p, uint8_t ch){
	if (!bufio_is_full(p)) {
		if(bufio_maxblk(p) == 0) bufio_shift(p);
		*(bufio_head(p)) = ch;
		bufio_extend(p, 1);
		return 1;
	}
...
size_t bufio_pull_bytes(bufio_t *p, void *t, size_t sz){
	if (bufio_used(p) < sz) sz = bufio_used(p);
